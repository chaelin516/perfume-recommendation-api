...
DATA_PATH = os.path.join(BASE_DIR, "../data/perfume_final_dataset.csv")
...
# ✅ 향수 추천 API (1차)
@router.post("/recommend", response_model=List[PerfumeRecommendItem],
             summary="1차 향수 추천", description="gender, season, time, impression, activity, weather 기반 향수 10개 추천")
def recommend_perfumes(request: RecommendRequest):
    ...
    user_emotions = ["clean", "sensual"]  # 기본 감정값 (변경 가능)
...
@router.post("/recommend-by-cluster", response_model=List[PerfumeRecommendItem],
             summary="클러스터 기반 향수 추천", description="감정 클러스터 ID(0~5)를 기반으로 향수 10개 추천")
...
@router.post("/predict-emotion", response_model=EmotionClusterResponse,
             summary="감정 클러스터 예측", description="입력 정보로부터 감정 클러스터 ID 예측")
